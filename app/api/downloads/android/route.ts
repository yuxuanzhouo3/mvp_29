...
export async function GET(req: NextRequest) {
  try {
    ...
    const variant = String(searchParams.get("variant") || "").trim()
    if (!variant || !["domestic", "international"].includes(variant)) {
      return NextResponse.json({ error: "variant 仅支持 domestic 或 international" }, { status: 400 })
    }
    if (isTencentTarget() && variant !== "domestic") {
      return NextResponse.json({ error: "当前环境仅支持国内版" }, { status: 400 })
    }
    if (!isTencentTarget() && variant !== "international") {
      return NextResponse.json({ error: "当前环境仅支持国际版" }, { status: 400 })
    }
    ...
    if (isTencentTarget()) {
      ...
      const fileId =
        typeof value === "object" && value && "fileId" in value ? String(value.fileId || "") : ""
      if (!fileId) {
        return NextResponse.json({ error: "尚未配置下载链接" }, { status: 404 })
      }
    ...
    const path =
      typeof value === "object" && value && "path" in value ? String(value.path || "") : ""
    if (!path) {
      return NextResponse.json({ error: "尚未配置下载链接" }, { status: 404 })
    }
